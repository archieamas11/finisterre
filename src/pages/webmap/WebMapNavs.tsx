      ...
      {isAdmin() && location.pathname === '/admin/map' && (
        <>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuItem onClick={onAddMarkerClick}>
                {isAdminContext(locateCtx) && locateCtx.isAddingMarker
                  ? 'Cancel Add'
                  : 'Add Marker'}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={onEditMarkerClick}>
                {isAdminContext(locateCtx) && locateCtx.isEditingMarker
                  ? 'Cancel Edit'
                  : 'Edit Marker'}
              </DropdownMenuItem>
            </DropdownMenuContent>
      ...
      {!isAuthenticated() && (
        <Link to='/login'>
          <Button
            variant='secondary'
            size='default'
            className='lg:size-default md:size-icon sm:size-icon rounded-full transition-all duration-200 lg:gap-2'
          >
