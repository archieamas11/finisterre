  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span
        ...
        title={row.original.username ?? String(row.original.user_id)}
      >
        <div
          className='bg-muted flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-sm font-medium'
          aria-hidden='true'
        >
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const fmt = isNaN(d.getTime())
        ? String(row.original.created_at)
        : d.toLocaleString()
