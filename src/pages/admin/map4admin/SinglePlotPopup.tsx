...
export default function SinglePlotLocations({
  marker,
  popupCloseTick
}: PlotLocationsProps) {
  ...
  type DeceasedItem = Partial<
    Pick<DeceasedType, 'dead_fullname' | 'dead_interment' | 'deceased_id'>
  >
  ...
  const deceasedList: DeceasedItem[] = Array.isArray(deceasedData)
    ? deceasedData.map((d: unknown) => {
        ...
        return {
          deceased_id:
            typeof dd?.deceased_id === 'number' ? dd.deceased_id : undefined,
          dead_fullname:
            typeof dd?.dead_fullname === 'string'
              ? dd.dead_fullname
              : undefined,
          dead_interment:
            typeof dd?.dead_interment === 'string'
              ? dd.dead_interment
              : undefined
        } as DeceasedItem
      })
  ...
  async function handleSaveOwner() {
    ...
    try {
      ...
      const customer = (customers as Customer[]).find(
        (c) => String(c.customer_id) === String(selectedCustomer)
      )
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to save owner'
    ...
    <div className='max-w-full'>
        ...
        <div className='bg-card relative mb-3 rounded-lg border p-3'>
          ...
          <Popover open={comboOpen} onOpenChange={setComboOpen}>
            <PopoverTrigger asChild className='bg-accent text-muted-foreground'>
              ...
              >
                {selectedCustomer
                  ? (() => {
                      const c = (customers as Customer[]).find(
                        (x) =>
                          String(x.customer_id) === String(selectedCustomer)
                      )
                      return c
                  ...
                  <CommandGroup>
                    {(customers as Customer[]).map((c: Customer) => (
                      <CommandItem
                        ...
                        onSelect={() =>
                          handleCustomerSelect(String(c.customer_id))
                        }
                        key={c.customer_id}
                      >
                        {c.first_name} {c.last_name} | ID: {c.customer_id}
                        <Check
                          className={cn(
                            'ml-auto h-4 w-4',
                            String(selectedCustomer) === String(c.customer_id)
                              ? 'opacity-100'
                              : 'opacity-0'
                          )}
                        />
          ...
          <div className='mt-3 flex gap-2'>
            ...
            <Button
              ...
              disabled={isSavingOwner || !selectedCustomer}
              size='sm'
              className='flex-1 leading-none'
            >
      ...
      {ownerData && (
        <CreateDeceasedRecordDialog
          ...
          initialValues={
            ownerData
              ? ({ lot_id: ownerData?.lot_id } as unknown as DeceasedRecords)
              : undefined
          }
          isPending={false}
          mode='add'
