...
export default function MapStats() {
  ...
  const { data: serenity, isLoading: isSerenityLoading, error: serenityError } = useQuery({ queryKey: ["map-stats", "serenity"], queryFn: getSerenityStats, staleTime: 60_000 });

  const { data: chambers, isLoading: isChambersLoading, error: chambersError } = useQuery({ queryKey: ["map-stats", "chambers"], queryFn: getChambersStats, staleTime: 60_000 });
  ...
  const locationData = [
    {
      ...
      name: "Serenity Lawn",
      stats: [
        { label: "Available", value: serenity?.available ?? 0, color: "text-green-600", bgColor: "bg-green-100 dark:bg-green-900/30" },
        { label: "Occupied", value: serenity?.occupied ?? 0, color: "text-red-600", bgColor: "bg-red-100 dark:bg-red-900/30" },
        { label: "Reserved", value: serenity?.reserved ?? 0, color: "text-yellow-600", bgColor: "bg-yellow-100 dark:bg-yellow-900/30" },
      ],
    ...
    },
    {
      ...
      stats: [
        { label: "Available", value: chambers?.available ?? 0, color: "text-green-600", bgColor: "bg-green-100 dark:bg-green-900/30" },
        { label: "Occupied", value: chambers?.occupied ?? 0, color: "text-red-600", bgColor: "bg-red-100 dark:bg-red-900/30" },
        { label: "Reserved", value: chambers?.reserved ?? 0, color: "text-yellow-600", bgColor: "bg-yellow-100 dark:bg-yellow-900/30" },
      ],
  ...
  const StatsCard = () => (
    <Card className="w-full overflow-hidden border border-white/20 bg-[#f1eff5]/90 p-0 shadow-xl backdrop-blur-md dark:border-stone-700/50 dark:bg-[#16141e]/80">
      ...
      <div className="border-b border-gray-200 px-6 pt-5 pb-3 dark:border-stone-700">
        ...
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Real-time availability data</p>
      </div>

      {/* Summary Stats */}
      <div className="bg-gray-50 px-6 py-5 dark:bg-stone-800/50">
        {/* Loading / Error States */}
        {isLoading && <div className="text-xs text-gray-500 dark:text-gray-400">Loading statistics…</div>}
        {hasError && !isLoading && <div className="text-xs text-red-600 dark:text-red-400">Failed to load some statistics.</div>}
