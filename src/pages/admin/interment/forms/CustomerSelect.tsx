...
export default function CustomerSelect({
  ...
  const selectedCustomer = customers.find(
    (c) => String(c.customer_id) === comboValue
  )

  return (
    <Popover
      onOpenChange={disabled ? undefined : setComboOpen}
      open={comboOpen}
    >
      <PopoverTrigger asChild>
        <Button
          ...
          role='combobox'
        >
          {comboValue && selectedCustomer
            ? `${selectedCustomer.first_name} ${selectedCustomer.last_name} | ID: ${comboValue}`
            : 'Select a customer'}
                ...
                {customers.map((c) => (
                  ...
                  >
                    ...
                    <CheckIcon
                      className={cn(
                        'ml-auto',
                        comboValue === String(c.customer_id)
                          ? 'opacity-100'
                          : 'opacity-0'
                      )}
                    />
