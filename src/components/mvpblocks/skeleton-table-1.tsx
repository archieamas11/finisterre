...
export const ShimmerTable: React.FC<ShimmerTableProps> = ({ rowCount = 5, columnCount = 5, renderHeading, columnWidthArray }) => {
...
const ShimmerTopBar: React.FC<{
  ...
  <div className="mb-2 flex w-full flex-col overflow-hidden rounded-md">
    <div className="flex h-15 items-center justify-between gap-4">
      {showFilter && <ShimmerComponent className="h-6 w-6/12 md:h-8 lg:w-3/12" />}
      {showColumnToggle && <ShimmerComponent className="ml-auto h-6 w-2/12 md:h-8 lg:w-1/12" />}
    </div>
  </div>
)
...
export default function SkeletonTableOneWrapper({
  ...
  return (
    <div className={`w-full ${bodyClassName}`}>
      {showTopBar && <ShimmerTopBar showFilter={showFilter} showColumnToggle={showColumnToggle} />}
      <ShimmerTable rowCount={rowCount} columnCount={columnCount} columnWidthArray={columnWidthArray} />
    </div>
  )
}
