...
export default function EditableMarker({
  ...
  const markerIcon = useMemo(() => {
    ...
    if (typeof icon.options.html === 'string') {
      ...
      icon.options.html &&
      typeof icon.options.html === 'object' &&
      'outerHTML' in icon.options.html
    ) {
  ...
  const updateCoordinatesMutation = useMutation({
    ...
    onMutate: async ({ plot_id, coordinates }) => {
      ...
      const previousPlots = queryClient.getQueryData<ConvertedMarker[]>([
        'plots'
      ])
      const previousPlotsClone = previousPlots
        ? JSON.parse(JSON.stringify(previousPlots))
        : undefined
  ...
  return (
    <Marker
      ...
      draggable={isSelected && isEditable}
    >
