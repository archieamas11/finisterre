...
function MapLibre() {
  ...
  const [directionsControl, setDirectionsControl] =
    useState<MapLibreGLDirections | null>(null);
  ...
  const showRoute = useCallback(
    async (from: [number, number], to: [number, number]) => {
      if (!directionsControl) {
        console.log(
          "⚠️ Directions control not available for route calculation",
        );
        return;
      }
      ...
      try {
        ...
        setDirectionsError(
          "Failed to calculate route. This might be due to network issues or the route service being unavailable.",
        );
      } finally {
  ...
  const handleDirectionsClick = useCallback(async () => {
    ...
    if (!directionsControl) {
      ...
      setDirectionsError(
        "Directions service is still loading. Please wait a moment and try again.",
      );
      return;
    }
        ...
        try {
          ...
          setTimeout(() => {
            try {
              ...
              setDirectionsControl(directions);
              console.log(
                "✅ Directions control initialized successfully (retry)",
              );
            } catch (retryError) {
              console.error(
                "❌ Failed to initialize directions control after retry:",
                retryError,
              );
              setDirectionsError(
                "Failed to initialize directions service. Please refresh the page.",
              );
            }
          }, 2000);
        }
      ...
      if ("geolocation" in navigator) {
        watchId = navigator.geolocation.watchPosition(
          ...
          (error) => {
            ...
            setDirectionsError(
              "Location tracking failed. Please check your permissions.",
            );
          },
          {
            timeout: 10000,
        ...
        {selectedMarker && (
          ...
          >
            <div className="w-72 rounded-lg border border-gray-200 bg-white shadow-lg">
              {/* Header */}
              <div className="rounded-t-lg bg-yellow-600 px-4 py-2 text-white">
              ...
              <div className="space-y-3 p-4">
                <div className="text-sm text-gray-600">
                  <div>
                    <strong>Status:</strong> {selectedMarker.plotStatus}
                  </div>
                  <div>
                    <strong>Category:</strong> {selectedMarker.category}
                  </div>
                  <div>
                    <strong>Area:</strong> {selectedMarker.dimensions.area}m²
                  </div>
                </div>
