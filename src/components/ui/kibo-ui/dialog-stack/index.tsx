...
import type { ButtonHTMLAttributes, Dispatch, HTMLAttributes, MouseEvent, MouseEventHandler, ReactElement, SetStateAction } from "react";
import { Children, cloneElement, createContext, useCallback, useContext, useEffect, useState } from "react";
...
export const DialogStack = ({ children, className, open, defaultOpen = false, onOpenChange, clickable = false, ...props }: DialogStackProps) => {
...
export const DialogStackBody = ({ children, className, ...props }: DialogStackBodyProps) => {
  ...
  return (
    ...
    >
      <Portal.Root>
        <div className={cn("pointer-events-none fixed inset-0 z-50 mx-auto flex w-full max-w-lg flex-col items-center justify-center", className)} {...props}>
...
export const DialogStackContent = ({ children, className, index = 0, offset = 10, ...props }: DialogStackContentProps) => {
  const context = useContext(DialogStackContext);
  ...
  return (
    ...
    <div
      className={cn("bg-background h-auto w-full rounded-lg border p-6 shadow-lg transition-all duration-300", className)}
      onClick={handleClick}
      style={{
        top: 0,
      ...
      {...props}
    >
      <div className={cn("h-full w-full transition-all duration-300", context.activeIndex !== index && "pointer-events-none opacity-0 select-none")}>{children}</div>
    </div>
  );
};
...
export const DialogStackNext = ({ children, className, asChild, ...props }: DialogStackNextProps) => {
  const context = useContext(DialogStackContext);
...
export const DialogStackPrevious = ({ children, className, asChild, ...props }: DialogStackPreviousProps) => {
  const context = useContext(DialogStackContext);
